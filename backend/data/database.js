...
db.serialize(() => {
  db.run("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT, password TEXT, role TEXT, teamId INTEGER, FOREIGN KEY(teamId) REFERENCES teams(id))");
  ...
  db.run("CREATE TABLE IF NOT EXISTS tasks (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, description TEXT, status TEXT, assignedTo INTEGER, teamId INTEGER, FOREIGN KEY(assignedTo) REFERENCES users(id), FOREIGN KEY(teamId) REFERENCES teams(id))");
